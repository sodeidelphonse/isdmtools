...
#'
#' @param test.data A named `list` of `sf` objects. Each `sf` object represents a different test dataset and must contain point geometries. The function will loop through each named dataset in the list. In particular, `test.data` can be a 'fold' from the \link{create_folds} and \link{extract_fold} outputs, if independent validation datasets are not available.
...
#' @param expected.response A `SpatRaster` object containing the model's predictions on a continuous scale (i.e. counts or rate if offset is used; see \lin{suitability_index}). Its values are used to compute all continuous-outcome metrics (e.g., RMSE, MAE, MAPE). This argument is required if a continuous-outcome metric is requested.
...
#' @param n.background integer. It specifies the number of pseudo-absence points to sample for presence-only data. Default is 1000 (see \link{sample_background}).
...
#' @param responsePA character. The column name in the `sf` objects that contains presence-absence data (1 for presence, 0 for absence). Default is 'present' and must be standardized across all PA data sets.
...
#' @param threshold.method character. The method to be used for selecting the threshold for converting probabilities to binary outcomes. Options are 'best' (using `best.method`) or 'fixed'. Default is "best".
#' @param best.method character. The method to be used for selecting the best threshold when `threshold.method` is 'best'. Options are 'youden' or 'closest.topleft'. Default is "youden" criterion which maximizes the sensitivity and specificity.
#' @param fixed.threshold numeric. The value (0-1) to be used as the fixed threshold when `threshold.method` is 'fixed'. Default is `NA_real_`.
#' @param best.threshold.policy character. Specifies the policy for selecting a threshold when multiple thresholds yield the same 'best' value. Options are "first", "last", "max.prec" (max precision), "max.recall" (max recall), "max.accu" (max accuracy), or "max.f1" (max F1 score). Default is "first".
#' @param metrics character. A vector of the metric names to compute. If `NULL`, "auc" (area under the ROC curve), "tss" (true skill statistics), "accuracy", "F1" (F1 score), "precision", and "recall" are computed for ROC-based metrics while "rmse" (root mean squared error), "mae" (mean absolute error) and "r2" (pseudo R-squared) are computed for error-based metrics.
#' @param overall.roc.metrics character. A vector of a subset of ROC-based metrics to be used for the overall composite score (`TOT_ROC_SCORE`). Allowed options are "auc", "tss", "accuracy", and "F1". If `NULL`, the sensible default is "auc", "tss" and "accuracy".
...
#' @param overall.error.metrics character. A vector of a subset of continuous outcome metrics to be used for the overall composite score (`TOT_ERROR_SCORE`). Allowed options are "rmse", "mae", and "r2".
...
#' @param is.pred.rate logical. If `TRUE`, it indicates that the `expected.response` contains predictions at the intensity (per-unit-of-exposure) scale (typical for Bayesian models with offset from `inlabru`). If `FALSE`, it assumes predictions are at the original scale (e.g., counts). Default is `FALSE`.
