...
\end{aligned}
$$
where $\mathrm{IPP}$ means a _Inhomogeneous Poisson Process_ and $\mathbf{s}$ the vector of a location coordinates. The IPP model presented here is known in spatial statistics as a log-Gaussian Cox process model - LGCP (see, @moller_log_1998) . Although the large component which includes the data-specific intercept can also incorporate environmental covariates, we assume that the basic model above is valid for the data.
...
```{r mesh-code, fig.width = 3, fig.height = 4, out.width = "90%", eval = exists("ben_sf")} 
