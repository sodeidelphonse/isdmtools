...
You can now prepare the remaining data required to fit an integrated model. First, we need to define the mesh to be used to approximate the latent field as well as for the integration points in the IPP likelihood.
After this step, we set up the prior distributions for the latent parameters such as the range and the marginal standard deviation. Then we define the observation model for each data type to fuse them using a joint likelihood estimation with INLA.
...
# Suitability analysis and evaluation with `isdmtools`
